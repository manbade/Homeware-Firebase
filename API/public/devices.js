...
database.ref('status').on('value', statusSnapshot => {
  database.ref('devices').once('value', devicesSnapshot => {
    database.ref('token').on('value', tokensSnapshot => {
      var status = statusSnapshot.val();
      var devices = devicesSnapshot.val();
      var tokens = tokensSnapshot.val();
      var scenesHTML = '';
      var html = '';
      Object(devices).forEach(function(device){
        ...
        if (device.traits[0] == 'action.devices.traits.Scene'){
          ...
          //Get alerts
          var alert = '';
          if(tokens[device.id]['authorization_code']['requestManualAuthorization']){
            ...
            html += '<div class="card-body">';
              html += '<div class="grid" style="margin-bottom:10px;">';
                ...
                html += '<div class="row">';
                  ...
                  html += '<div class="col" style="vertical-align:top; text-align:right;"><a href="/cms/devices/edit/?id=' + device.id + '" class="btn btn-primary">Edit</a>' + alert + '</div>';
                html += '</div>';
              html += '</div>';
            html += '</div>';
          html += '</div>';
